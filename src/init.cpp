...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: INSaNe.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: INSaNed.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeINSaNeamount=<n> " + _("Keep N INSaNe anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. INSaNe is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. INSaNe is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("INSaNe version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "INSaNe server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of INSaNe") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart INSaNe to complete") << "\n";
    ...
    nAnonymizeINSaNeAmount = GetArg("-anonymizeINSaNeamount", 0);
    if(nAnonymizeINSaNeAmount > 999999) nAnonymizeINSaNeAmount = 999999;
    if(nAnonymizeINSaNeAmount < 2) nAnonymizeINSaNeAmount = 2;
    ...
    LogPrintf("Anonymize INSaNe Amount %d\n", nAnonymizeINSaNeAmount);
